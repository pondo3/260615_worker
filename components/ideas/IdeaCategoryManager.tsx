...
import { createIdeaCategory, updateIdeaCategory, deleteIdeaCategory, IdeaCategoryFormState } from '@/app/actions/ideas'
...
function CategoryForm({
  ...
  const action = editing ? updateIdeaCategory : createIdeaCategory
    ...
    <form action={formAction} className="bg-gray-50 dark:bg-gray-800/50 rounded-xl p-4 space-y-3 border border-gray-200 dark:border-gray-700">
      ...
      <div>
        <input
          ...
          placeholder="카테고리명 *"
          className="w-full text-sm bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-700 dark:text-gray-300 placeholder-gray-400 dark:placeholder-gray-500 outline-none focus:border-indigo-400 transition-colors"
        />
      ...
      <div>
        ...
        <input
          ...
          min="0"
          className="w-24 text-sm bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-lg px-3 py-2 text-gray-700 dark:text-gray-300 outline-none focus:border-indigo-400 transition-colors"
        />
      </div>
      ...
      <div className="flex gap-2 pt-1">
        ...
        <button type="submit" disabled={pending}
          className="flex-1 py-2 rounded-lg bg-indigo-500 text-xs font-bold text-white hover:bg-indigo-600 transition-colors disabled:opacity-50">
...
export default function IdeaCategoryManager({ onClose, categories }: Props) {
  ...
  function handleDelete(cat: Category) {
    if (cat._count.ideas > 0) {
      setDeleteError({ id: cat.id, msg: `이 카테고리에 아이디어 ${cat._count.ideas}개가 있습니다. 먼저 다른 카테고리로 이동하거나 삭제해주세요.` })
      return
    }
    startTransition(async () => {
      const result = await deleteIdeaCategory(cat.id)
                ...
                <div className="flex items-center gap-3 p-3 bg-gray-50 dark:bg-gray-800/50 rounded-xl border border-gray-200 dark:border-gray-700 group">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-gray-400 dark:text-gray-500 flex-shrink-0">{cat._count.ideas}개</span>
                    </div>
                  </div>
        ...
        <div className="px-5 pb-4 border-t border-gray-100 dark:border-gray-800 pt-3 flex-shrink-0">
          {!showAddForm && editingId === null && (
            <button
              ...
              className="w-full py-2.5 rounded-xl border-2 border-dashed border-indigo-300 dark:border-indigo-700 text-sm font-semibold text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-900/20 transition-colors"
            >
              + 카테고리 추가
            </button>
          )}
