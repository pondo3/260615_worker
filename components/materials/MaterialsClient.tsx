      ...
      <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-2xl w-full max-w-2xl mb-10">
        ...
        <div className="px-6 py-5 space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div>
              <label className={label}>한글 제목 {form.isTranslated && <span className="text-blue-400">(자동 번역)</span>}</label>
              <input value={form.translatedTitle} onChange={(e) => setForm((p) => ({ ...p, translatedTitle: e.target.value, translationEdited: true }))} className={input} />
            </div>
          </div>
  ...
  return (
    ...
    >
      ...
      <td className="px-2 py-2 max-w-[180px]">
        <p className="text-xs text-gray-900 dark:text-white font-medium line-clamp-2 leading-snug">{mat.originalTitle || '-'}</p>
      </td>
      <td className="px-2 py-2 max-w-[160px]">
        <p className="text-xs text-gray-500 dark:text-gray-400 line-clamp-2 leading-snug">{mat.translatedTitle || '-'}</p>
      </td>
      <td className="px-2 py-2">
      ...
      {tab === 'list' && (
        <>
          ...
          <div className="flex-shrink-0 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 px-6 py-2.5 flex items-center gap-3 flex-wrap">
            ...
            {(filterPlatform !== 'all' || filterUsage !== 'all' || filterStatus !== 'all' || search) && (
              <button onClick={() => { setFilterPlatform('all'); setFilterUsage('all'); setFilterStatus('all'); setSearch('') }}
                ...
                <table className="w-full text-left border-collapse min-w-[1200px]">
                  <thead className="bg-gray-50 dark:bg-gray-900 sticky top-0 z-10">
                    <tr>
                      ...
                      {['썸네일','플랫폼','원본 제목','한글 제목','채널','조회수','좋아요','등록일','업로드일','사용여부','예정일','사용일','상태','태그','링크'].map((h) => (
